...
def authenticate(cl: Client, config: dict, conn):
    c = conn.cursor()
    c.execute(
        "CREATE TABLE IF NOT EXISTS session (id INTEGER PRIMARY KEY, session_json TEXT)"
    )
    conn.commit()
    ...
    try:
        ...
        logging.error(
            "🚨 Instagram triggered a challenge (e.g. phone verification). Manual login required."
        )
        raise SystemExit("Bot stopped: challenge_required")

    session_data = json.dumps(cl.get_settings())
    c.execute(
        "INSERT OR REPLACE INTO session (id, session_json) VALUES (1, ?)",
        (session_data,),
    )
    conn.commit()
